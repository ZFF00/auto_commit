...
- `auto_commit_v2.py`：调用分析器，安全地更新 `.gitignore`、暂存批准的文件、提交到本地、推送到远程仓库并发送邮件通知；也可以作为常驻定时任务运行。

原版 `auto_commit.py` 保持不变。
...
```powershell
git remote add origin git@github.com:ZFF00/auto_commit.git
```
...
```powershell
python auto_commit_v2.py --once --dry-run --live
```
...
```powershell
python auto_commit_v2.py --once --live
```

指定仓库、远程仓库和目标分支：

```powershell
python auto_commit_v2.py --once --repo D:\work\project --remote origin --branch main --live
```

只创建本地提交、不推送：

```powershell
python auto_commit_v2.py --once --no-push
```
...
```powershell
python auto_commit_v2.py --once --mail-sender "sender@qq.com" --mail-auth-code "SMTP授权码" --mail-recipients "receiver@example.com"
```
...
```powershell
python auto_commit_v2.py --live
```

每天 `09:00` 和 `18:00` 执行，并在启动时先执行一次：

```powershell
python auto_commit_v2.py --time 09:00 --time 18:00 --run-now --live --log-file auto_commit_v2.log
```

时间也可以写成一个逗号分隔参数：

```powershell
python auto_commit_v2.py --time 09:00,18:00
```

常驻模式需要终端或后台进程持续运行。也可以使用 Windows 任务计划程序定时调用 `python auto_commit_v2.py --once`，此时调度由 Windows 管理，脚本仍负责完整的单次分析、提交和推送流程。
...
- 任务异常退出后如果留下 `.git/auto_commit_v2.lock`，确认没有其他任务运行后再人工删除该锁文件。
...
python -m py_compile auto_commit_v2.py codex_git_advisor.py email_notifier.py test_auto_commit_v2.py test_codex_git_advisor.py test_email_notifier.py
```
